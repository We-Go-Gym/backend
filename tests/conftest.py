...
def db_session():
    """
    Fixture principal: Roda para CADA teste.
    Cria tabelas, cede a sessão, e dropa tabelas.
    Funciona perfeitamente RÁPIDO por estar em memória.
    """
...
def aluno_cadastrado(client):
    ...
    response = client.post(
        "/Aluno/",
        json={
            "nome_aluno": "Aluno Padrão Fixture",
            "email": "fixture.aluno@email.com",
            "senha": "senha123",
            "idade": 25,
            "peso_kg": 75.0,
            "altura": 1.75
...
def exercicio_cadastrado(client):
    ...
    response = client.post(
        "/Exercicio/",
        json={
            "nome_exercicio": "Supino Reto (Fixture)",
